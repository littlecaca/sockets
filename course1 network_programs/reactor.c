...
#define CONNECTION_LENGTH 1048576
...
int recv_cb(int fd)
{
    ...
    // printf("from %d: %s", clnt_fd, pconn->rbuffer);

    // echo
    pconn->wlength = pconn->rlength;
    memcpy(pconn->wbuffer, pconn->rbuffer, pconn->rlength);
    set_event(clnt_fd, EPOLLIN | EPOLLOUT, EPOLL_CTL_MOD);
